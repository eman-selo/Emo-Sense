  ...
  if (!emotionClassifierModel) {
    ...
    try {
      ...
      if (modelsInDB[`indexeddb://${EMOTION_MODEL_STORAGE_NAME}`]) {
        emotionClassifierModel = await loadLayersModel(
          `indexeddb://${EMOTION_MODEL_STORAGE_NAME}`
        );
        console.log("EmotionClassifier has been loaded from IndexedDB");
      } else {
        console.log(
          "EmotionClassifier model not found in IndexedDB, loading from network..."
        );
        emotionClassifierModel = await loadLayersModel(
          EMOTION_CLASSIFICATION_MODEL_URL
        );
        console.log("EmotionClassifier has been loaded from network");
        await emotionClassifierModel.save(
          `indexeddb://${EMOTION_MODEL_STORAGE_NAME}`
        );
      ...
      console.error(
        "Failed to load or save the EmotionClassifier model",
        error
      );
      throw new Error(
        `Failed to load EmotionClassifier model: ${error.message}`
      );
    }
  }
  ...
  if (!blazefaceModel) {
    ...
    try {
      ...
      blazefaceModel = await blazeface.load({
        modelUrl: localBlazefaceModelPath,
      });
      console.log("blazeface model has been loaded");
    } catch (error) {
      console.error("failed to load blazeface model", error);
      throw new Error(
        "blazeface model loading failed. Please check the model path."
      );
    }
  }
};
...
const getFacesRects = async (videoElement) => {
  ...
  const facePositions = await blazefaceModel.estimateFaces(
    videoElement,
    returnTensors,
    flipHorizontal,
    annotateBoxes
  ...
  for (const position of facePositions) {
    ...
    position.bottomRight[0] = Math.min(
      videoElement.videoWidth,
      position.bottomRight[0]
    );
    position.bottomRight[1] = Math.min(
...
const getFaceImage = async (imgTensor, position) => {
  const imgDimensions = tf.tensor2d(
    [imgTensor.shape[1], imgTensor.shape[2]],
    [1, 2]
  );

  const normalizedTopLeft = tf
    .div(tf.tensor1d([position.topLeft[1], position.topLeft[0]]), imgDimensions)
    .dataSync();
  const normalizedBottomRight = tf
    .div(
      tf.tensor1d([position.bottomRight[1], position.bottomRight[0]]),
      imgDimensions
    )
    .dataSync();

  const box = tf
    .concat([normalizedTopLeft, normalizedBottomRight])
    .expandDims(0);
  const faceImage = tf.image.cropAndResize(
    imgTensor,
    box,
    [0],
    CLASSIFIER_IMG_DIMENSIONS
  );
  const normalizedFaceImage = faceImage.div(tf.scalar(255));

  return normalizedFaceImage.mean(3, false).toFloat().expandDims(3);
};
...
const classifyEmotion = async (img) => {
  ...
  const indexOfMaxValue = predictions.reduce(
...
const EmotionDetector = () => {
      ...
      intervalIdRef.current = setInterval(async () => {
        ...
        try {
          ...
          if (facePositions.length > 0) {
            const faceProcessingPromises = facePositions.map(
              async (position) => {
          ...
          console.error(
            "Error in the detection or classification process : ",
            error
          );
        } finally {
          imgTensor.dispose();
  ...
  return (
    <div className="flex flex-col items-center p-8 bg-white rounded-xl shadow-lg max-w-4xl mx-auto my-6">
      ...
      <div
        className="relative w-full max-w-2xl mx-auto border-2 border-gray-300 rounded-lg overflow-hidden bg-black flex justify-center items-center min-h-96 mb-6"
        style={{ display: loadingModels ? "flex" : "block" }}
      >
        {loadingModels && (
          <p className="text-gray-600 text-lg italic p-4">
            Loading machine learning models ...Please wait{" "}
          </p>
        )}
        <video
          ref={videoRef}
      ...
      <h2 className="text-2xl font-bold text-gray-700 mt-5 mb-4 text-center">
        Detected Faces
      </h2>
      <div className="flex flex-wrap justify-center gap-4 p-4 border border-gray-200 rounded-lg bg-gray-50 max-w-full">
        {detectedFacesData.length > 0 ? (
          detectedFacesData.map((face, index) => (
            <div
              key={index}
              className="flex flex-col items-center p-3 border border-gray-300 rounded-md shadow-sm bg-white"
            >
              <img
                src={face.imageDataUrl}
                alt={`Detected Face ${index}`}
                className="w-20 h-20 object-cover rounded-sm mb-2 border border-gray-200"
              />
